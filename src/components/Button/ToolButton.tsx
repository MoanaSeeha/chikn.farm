   ...
   props.buttonProps.map((btn, i) => {
    return (
     <div 
      key={i}
      className={
        (props.buttonProps.length === 1?'rounded-lg': 
          (i !== 0)?
          (i === props.buttonProps.length-1? 'rounded-r-lg':''):
            'rounded-l-lg') + ' p-2 w-10 h-10 border border-[#3b3d76] bg-[#222343] color-[#f2f2f2] transition-all hover:bg-[#2a2c54] cursor-pointer text-base'}>
          {btn.title?btn.title:<></>}
