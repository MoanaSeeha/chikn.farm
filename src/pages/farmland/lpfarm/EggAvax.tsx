  ...
  return (
      <Board className={`relative pb-0`}>
        <div onClick={() => toggle(!toggleStatus)} className={`${!toggleStatus?'rounded-br-lg rounded-bl-lg ':''}items-center cursor-pointer absolute bg-[#222343] rounded-tr-lg rounded-tl-lg w-full top-0 flex justify-between px-5 py-2`}>
          ...
          <img src={`${process.env.PUBLIC_URL}/img/eggtoken.svg`} alt="" className='w-8 h-8'/>
            <div className='text-2xl font-black tracking-widest'>$EGG /</div>
        ...
        <SlideDown className={'my-dropdown-slidedown'}>
          {toggleStatus ? 
            <div className='px-3 flex md:flex-row flex-col space-x-0 md:space-x-5 py-5'>
              <div className='flex flex-col md:basis-2/5 px-5 bg-[#222343] rounded-lg py-8'>
                <div className='flex flex-row justify-start space-x-5 text-lg'>
                  <div className={`${selectedTab?'chikn-red ':''} cursor-pointer transition-all`} onClick={() => changeTab(true)}>Deposite</div>
                ...
                <div className='flex flex-col justify-center mt-4 space-y-5'>
                    <div className='flex justify-center py-2 px-5 bg-[#df3f3d] rounded-lg text-[#f2f2f2] text-base'>
                      TRADER JOE <img src={`${process.env.PUBLIC_URL}/img/JLP_Logo-482a1e1547fafda8e8b152d09098d3e3.png`} alt='' className=' w-6 h-6'/>
                    </div>
                    <a href="#" className='flex justify-center inline-block chikn-red underline decoration-[#df3f3d]'>Get JLP tokens here</a>
                    ...
                    <RoundedButton title='Deposite'/>
                </div>
              </div>
              <div className='flex flex-col justify-center text-center space-y-5 md:basis-3/5 px-10 chikn-red bg-[#222343] rounded-lg py-8'>
                <div>ACTIVATE YOUR LP FARMING OR DEPOSIT LP TO START</div> 
